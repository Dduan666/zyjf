...
//                  height: '100%'
            ...
            function move() {
                ...
                //图片切换
                $box.children('div').eq($index).stop().fadeIn(600, function () {
                    if ($flag) {
                        clearTimeout($timer);
                        $timer = setTimeout(move, o.times);
                    }
                }).siblings('div').stop().fadeOut(600);
                //焦点切换
                $oUl.children('li').eq($index).stop().fadeTo(600, 0.9).siblings('li').stop().fadeTo(600, 0.3);
            }
            //焦点切换
            $oUl.children('li').click(function () {
                    $index = $(this).index();
                    $box.children('div').eq($index).stop().fadeIn(600).siblings('div').stop().fadeOut(600);
                    $(this).stop().fadeTo(600, 0.9).siblings('li').stop().fadeTo(600, 0.3);
                });
                //左右点击切换
            $box.hover(function () {
                    $flag = false;
                    clearTimeout($timer);
                    $(this).children('h3').stop().fadeIn(600);
                    ...
                    $flag = true;
                    $timer = setTimeout(move, o.times);
                    $(this).children('h3').stop().fadeOut(600)
                });
                //阻止文本被选中
